...
parameters = [2,1,15000,100,[100],0.1]#40
...
class neural_network:
    ...
    def __init__(self,layer_shapes,activation):
        ...
        self.activation = activation
        for i in range(len(layer_shapes)-1):
            w = (np.random.rand(layer_shapes[i]+1, layer_shapes[i+1]))
            self.weights.append(w)
        self.weights = np.array(self.weights)
...
#gaber_train_arr = (np.array([nd.convolve(z,kernels[3],mode='wrap') for z in x_train_reshaped])).astype(float)
...
#gaber_test_arr = (np.array([nd.convolve(z,kernels[3],mode='wrap') for z in x_test_reshaped])).astype(float)
...
#parameters=[1,0.5,10000,100,[200,200],0.1,softplus]
#parameters=[2,1,20000,100,[100],0.1,softplus]#40
#parameters=[2,1,4000,100,[100],0.3,softplus]# canny 39 % 
#parameters=[2,1,10000,100,[80],0.3,softplus]# canny 39 % 
...
#parameters = [1,,20000,200,[100],0.1,sigmoid]
parameters=[2,1,20000,200,[100],0.1,softplus]#40
...
for i in range(num_iters):
    ...
    if(learning_type == 2):
        #learning_rate = learning_rate/np.sqrt(i+1)
        learning_rate = learning_rate/np.sqrt(epoch_number)
    nn.backpropagate(y,learning_rate,regularization_parameter)
    if (i+1)%1000==0:    
        ...
        train_val_acc.append([i+1,0,val_accuracy])
        print("iteration:{} train_acc:{} val_acc:{}".format(i+1,round(0,3),round(val_accuracy,3)))
